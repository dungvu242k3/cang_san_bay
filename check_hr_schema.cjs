...
async function main() {
    ...
    else console.log('employee_profiles COLUMNS:', profiles && profiles.length ? Object.keys(profiles[0]).join(', ') : 'Empty table')
    ...
    for (const table of guesses) {
        ...
        else console.log(`Table ${table} COLUMNS:`, data && data.length ? Object.keys(data[0]).join(', ') : 'Empty table')
    }
