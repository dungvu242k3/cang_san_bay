-- ==========================================================
-- FIX: TẠO BẢNG TASK_ATTACHMENTS (Force Create)
-- Chạy file này trong Supabase Dashboard > SQL Editor
-- ==========================================================

-- Tạo bảng task_attachments nếu chưa tồn tại
...
BEGIN
    IF EXISTS (
        ...
        IF EXISTS (
            ...
            RAISE NOTICE '✅ Bảng task_attachments đã được tạo thành công với cột file_path!';
        ELSE
            RAISE EXCEPTION '❌ Lỗi: Cột file_path không tồn tại trong bảng task_attachments';
        END IF;
    ELSE
        RAISE EXCEPTION '❌ Lỗi: Không thể tạo bảng task_attachments';
    END IF;
END $$;

SELECT '✅ Hoàn tất! Bảng task_attachments đã sẵn sàng.' as status;
